        ...
        if (strncmp("SENDFILE", buffer, 8) == 0)
        {
            if ((strcmp("mnop", atoken.c_str()) == 0))
            {
                ...
                for (int i = 0; i < extensions.size(); i++)
                {
                    ...
                    if (strcmp(substring.c_str(), "txt") == 0)
                    {
                        files += extensions[i];
...
int main()
{
    ...
    //
    string fServer = "servicename:Textserver,servicetype:text,ipaddress:127.0.0.1,portnum:8008,serviceaccesstoken:mnop";
    char buffer[SIZE];
