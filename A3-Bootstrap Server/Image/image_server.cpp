    ...
    else
    {
        ...
        if (strncmp("SENDFILE", buffer, 8) == 0)
        {
            if ((strcmp("ijkl", atoken.c_str()) == 0))
            {
                ...
                // appending all te available files to a string
                string files = "";
                for (int i = 0; i < extensions.size(); i++)
                {
                    ...
                    if (strcmp(substring.c_str(), "jpg") == 0)
                    {
                        files += extensions[i];
...
int main()
{
    ...
    //
    string fServer = "servicename:Imageserver,servicetype:image,ipaddress:127.0.0.1,portnum:8007,serviceaccesstoken:ijkl";
    char buffer[SIZE];
